...
class VLMAgent:
    ...
    def analyze(
        ...
        if hasattr(response, "usage") and response.usage:
            print(
                f"[VLM Agent] Tokens: Input={response.usage.prompt_tokens}, "
    ...
    def _resize_for_vision(
        self, image: Image.Image, max_edge: int = 1024
    ) -> Image.Image:
...
```"""
    ...
    def _parse_dual_output(self, response: str) -> Dict[str, str]:
        ...
        if "DETECTION_PROMPT:" in response and "GAME_CONTEXT:" in response:
            ...
            print(
                "[VLM Agent] Warning: Could not parse dual output format, using fallback"
            )
            game_context = response
            detection_prompt = "Detect: all interactive game elements"

        return {"detection_prompt": detection_prompt, "game_context": game_context}
