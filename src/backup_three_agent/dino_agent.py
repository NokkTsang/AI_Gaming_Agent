...
class DINOAgent:
    ...
    def _initialize_detector(self):
        ...
        try:
            ...
            if self.detector and self.detector.available:
                ...
                print("[DINO Agent] GroundingDINO not available")
        except Exception as e:
            print(f"[DINO Agent] FAILED to initialize detector: {e}")
            self.detector = None

    def detect(self, screenshot_path: str, detection_prompt: str) -> Dict[str, any]:
        """
        ...
        if not self.detector or not self.detector.available:
            print("[DINO Agent] Detector not available, returning empty results")
            return {
                "detected_objects": [],
        ...
        try:
            ...
            print(f"[DINO Agent] FAILED: Detection failed: {e}")
            return {
                "detected_objects": [],
                "screenshot_size": self._get_image_size(screenshot_path),
                "detection_prompt": detection_prompt,
    ...
    def _to_pixel_coords(
        self, detection: Dict, img_size: Dict[str, int]
    ) -> Dict[str, int]:
    ...
    def annotate_detections(
        ...
        try:
            ...
            if output_path is None:
                ...
                output_path = str(
                    Path(screenshot_path).parent / f"{base}_annotated.png"
                )
            ...
            print(f"[DINO Agent] FAILED: Annotation failed: {e}")
            return None
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        ...
        print(
            "Example: python -m src.modules.agents.dino_agent maze.png 'player, walls, goal'"
        )
        sys.exit(1)
    ...
    if result["detected_objects"]:
        ...
        for obj in result["detected_objects"]:
            ...
            print(
                f"  Position: ({obj['pixel_coords']['x_center']}, {obj['pixel_coords']['y_center']})"
            )
            print(
                f"  Size: {obj['pixel_coords']['width']} x {obj['pixel_coords']['height']}"
            )
