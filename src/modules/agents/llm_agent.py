...
class LLMAgent:
    ...
    def decide(
        ...
        if not self.client:
            print("[LLM Agent] ⚠️  OpenAI client not available")
            return {
                'action': None,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python -m src.modules.agents.llm_agent <game_context> <spatial_data_json>")
        print("\nExample:")
        print('  python -m src.modules.agents.llm_agent "Maze game" \'{"detected_objects":[]}\'')
        sys.exit(1)
